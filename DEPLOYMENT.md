...
- [Table of Contents](#table-of-contents)
  ...
  - [Local Development](#local-development)
    ...
    - [How to Create a Local Clone](#how-to-create-a-local-clone)
...
1. Go to your S3 bucket and create a new folder called "Media".
2. Inside the folder, click upload, add files and then select all your project images to add.
3. Scroll down to the Permissions detail and select 'Grant public-read access', check the risk statement then click the Upload button.
...
Remember to add the Stripe keys to the config vars in Heroku to keep them private.

1. Login to Stripe, click developers and then API keys.
2. In Heroku's config vars, create 2 new variables:

    - Publishable key: STRIPE_PUBLIC_KEY - paste values from the Stripe page.
    - Secret key: STRIPE_SECRET_KEY - paste values from Stripe

3. Next, add the webhook endpoint for the deployed site. In Stripe, navigate to the Webhooks tab and click the 'Add Endpoint' button. Add the URL for the deployed site, include `checkout/wh/` at the end. Select all events and click 'Create endpoint'.
4. Reveal and add the Webhook signing secret to the Heroku config vars, with this key STRIPE_WH_SECRET.
5. Make sure the Heroku variables match your settings.py:
...
1. Forking allows you to view and edit the code without affecting the original repository
2. Locate the GitHub repository. Link to this repository can be found [here](https://github.com/MoniPar/reclaimed-treasures).
3. Click on 'Fork', in the top right-hand corner.
4. This will take you to your own repository to a fork with the same name as the original branch.

_____

### How to Create a Local Clone

1. Go to the GitHub repository. Link to this repository can be found [here](https://github.com/MoniPar/reclaimed-treasures).
